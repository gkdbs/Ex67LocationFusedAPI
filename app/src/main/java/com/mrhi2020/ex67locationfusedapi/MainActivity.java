package com.mrhi2020.ex67locationfusedapi;
...
public class MainActivity extends AppCompatActivity {
    ...
    //Google 라이브러리 추가 : play-services-location

    GoogleApiClient googleApiClient; //위치정보관리 객체 ( LocationManager 역할 )
    FusedLocationProviderClient providerClient; //위치정보제공자 객체 (알아서 적절할 위치정보제공자를 선택)
    ...
    //requestPermissions()을 다이얼로그 선택완료시 자동 발동하는 콜백메소드
    @Override
        ...
        switch (requestCode) {
            case 0:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(this, "위치정보 제공에 동의 하셨습니다.", Toast.LENGTH_SHORT).show();
                } else {
    ...
    GoogleApiClient.ConnectionCallbacks connectionCallbacks = new GoogleApiClient.ConnectionCallbacks() {
        ...
        public void onConnected(@Nullable Bundle bundle) {
            ...
            //위치 정보 요청객체를 생성 및 설정 [Criteria : 제공자선택기준]
            LocationRequest locationRequest = LocationRequest.create();
            locationRequest.setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY);//높은 정확도 우선 : gps
            locationRequest.setInterval(5000); //위치정보 탐색 주기 : 5초마다

            //위치정보제공자 객체에게 실시간 위치정보 업데이트를 요청
    ...
    //액티비티가 화면에서 보이지 않으면 더이상 위치탐색 업데이트를 안하도록..
    @Override
